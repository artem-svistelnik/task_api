...
def get_products(product_id,page=1,per_page=1):
    ...
    product_reviews=Reviews.query.filter_by(products_asin=product_data.asin).paginate(page,per_page,True).items
...
def put_review(product_id):
    ...
    new_review=Reviews(products_asin=product.asin,title=request.json['title'],review=request.json['review'])
